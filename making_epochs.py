...
def load_good_lfp_data(dataframe_path, dataset_folder):
    ...
    good_channels = data[(data['Quality'] == 'Good') & (data['Area'] == 'A1')]
...
def create_and_save_epochs(lfp_data_list, good_indices, audio_timestamps, sleep_scores, save_folder, animal_id, condition):
    ...
    # Process data for each good channel
...
def main():
    ...
    if lfp_data_list is not None:
        ...
        create_and_save_epochs(lfp_data_list, good_indices, audio_timestamps, sleep_scores, save_folder, animal_id, condition)
